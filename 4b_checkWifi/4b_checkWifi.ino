...
void setup() {
  ...
  // client.publish("janfiess/msg", "_________restarted");
}
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  String id_topic_string = "janfiess/" + eeprom_value_string + "/anim";
  ...
  if (strcmp(topic, id_topic_char_array)==0){
    ...
    if ((char)payload[1] == '1') {
      // Serial.println("janfiess/1/anim -> payload 1");
      lightAnim1();
    }
    else if ((char)payload[1] == '2') lightAnim2();
    else if ((char)payload[1] == '3') runningLight1();
  ...
  String topic_id_static_string = "janfiess/" + eeprom_value_string + "/static";
...
void getId(){
  ...
  String eeprom_value_string = "Device id: ";  
  eeprom_value_string += String(eeprom_value);
  Serial.println(eeprom_value_string);

  str_len = eeprom_value_string.length() + 1;
  char eeprom_value_charArray[str_len];
  eeprom_value_string.toCharArray(eeprom_value_charArray, str_len);

  // client.publish("janfiess/msg", eeprom_value_charArray);
  eeprom_value_charArray[0] = 0;
}
...
void setId(String boardId_string){
  ...
  String old_topic_id_static_string = "janfiess/" + eeprom_value_string + "/static";
  ...
  String old_topic_id_anim_string = "janfiess/" + eeprom_value_string + "/anim";
  ...
  String newDeviceId_string = "New device ID: " + eeprom_value_string;
  ...
  client.publish("janfiess/msg", newDeviceId_charArray);
  ...
  String topic_id_anim_string = "janfiess/" + eeprom_value_string + "/anim";
...
void loop() {
  ...
  if((millis() - lastTimestamp) >= 15000){
    ...
    String heapsize_string = "Heap size: " + String(ESP.getFreeHeap());
    ...
    client.publish("janfiess/msg", heapsize_char_array);
  ...
  if(btn_fourClicks_setId == true){
    if((millis() - button_timestamp) >= 20000){
      ...
      client.publish("janfiess/msg", "Set id time expired");
    }
  }
  ...
  if(buttonState == 1){
    ...
    if(button_counter == 2){
      ...
      client.publish("janfiess/msg", "Set new device id");  
    }
