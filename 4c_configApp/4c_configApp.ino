...
void setId(String boardId_string){
  ...
  String newDeviceId_string = "New device ID: " + eeprom_deviceId_string;
...
void loop() {
  ...
  if(btn_fourClicks_setId == true){
    if((millis() - button_timestamp) >= 20000){
      ...
      client.publish("janfiess/msg", "Set id expired");
    }
  }
  ...
  if(buttonState == 1){
    ...
    if(button_counter == 2){
      ...
      client.publish("janfiess/msg", "Set id");  
    }
