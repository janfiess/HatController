...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic,"janfiess/getId")==0){
...
void getId(){
  ...
  String eeprom_value_string = String(eeprom_value);
  int str_len = eeprom_value_string.length() + 1;
  char eeprom_value_char_array[str_len];
  eeprom_value_string.toCharArray(eeprom_value_char_array, str_len);
  client.publish("janfiess/test", eeprom_value_char_array);
  
}

void setId(String boardId_string){
  // if button pressed three times
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("ESP8266Client")) {
      ...
      client.subscribe("janfiess/getId");
    } else {
